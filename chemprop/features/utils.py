...
def load_valid_node_or_edge_features(path: str, smiles: List[str]) -> List[np.ndarray]:
    """
    Loads features saved in a variety of formats.
    Supported formats:
    ...
    * :code:'.sdf' containing all mol blocks with descriptors as entries
    :param path: Path to file containing nodewise features.
    ...
    if extension == '.npz':
        ...
        if features_df.iloc[0, 0].ndim == 1:
            ...
            raise ValueError(f'Atom/edge descriptors input {path} format not supported')
